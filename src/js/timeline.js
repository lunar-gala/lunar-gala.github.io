...
    // Ensure the timeline container is not already populated
    if ($timeline.children.length === 0) {
        const yearKeys = Object.keys(years);
        console.log('Years fetched:', yearKeys); // Debugging log to verify fetched years

        // Loop through the years and append the slides
        yearKeys.reverse().forEach((year) => {
            const datum = years[year];

            // Log to detect any duplication or issues
...
        });
    } else {
        console.warn('Timeline is already populated, skipping re-append.');
    }
